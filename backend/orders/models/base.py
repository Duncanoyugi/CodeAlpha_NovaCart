...
class Order(models.Model):
    ...
    # Billing information (same as shipping if not provided)
    ...
    def save(self, *args, **kwargs):
        if not self.order_number:
            # Generate unique order number: ORD-20241224-ABC123
    ...
    @property
    def can_refund(self):
        return self.payment_status == self.PaymentStatus.PAID and self.status not in [self.Status.REFUNDED, self.Status.CANCELLED]
...
class OrderItem(models.Model):
    ...
    product_image = models.URLField()                # Snapshot
